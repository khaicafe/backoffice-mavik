...
func GetProducts(c *gin.Context) {
	...
	if err := models.DB.Preload("ProductOptions.TemperatureSize.Temperature").
		Preload("ProductOptions.TemperatureSize.SizeOption").
	...
	// Chuyển đổi các sản phẩm thành định dạng JSON như bạn yêu cầu
		...
		if _, exists := temperatureMap[tempID]; !exists {
			temperatureMap[tempID] = map[string]interface{}{
				...
				"size_options": []map[string]interface{}{
					{
						"SizeOptionId": option.TemperatureSize.SizeOption.ID,
						"name":         option.TemperatureSize.SizeOption.Name,
	...
	for _, option := range input.Options {
		for _, choice := range option.Choices {
			for _, sizeOption := range choice.SizeOptions {
				...
				models.DB.Create(&temperatureSize)
